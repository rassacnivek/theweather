    ...
    if (search !== "") {
      ...
      if (splitSearch[1] && splitSearch[1] !== "") {
        ...
        citiesFiltered = cities.filter(
          (city) =>
            city.name.toLowerCase().indexOf(splitSearch[0].toLowerCase()) >
              -1 && city.country.indexOf(splitSearch[1].toUpperCase()) > -1
        );
      } else {
        citiesFiltered = cities.filter(
          (city) =>
            city.name.toLowerCase().indexOf(splitSearch[0].toLowerCase()) > -1
        );
      }
      ...
      <div className="search__container">
        <span className="search__container-input__container">
          <input
            ...
            className="search__container-input__container-input"
            placeholder="Ex: New York, US"
            onChange={(e) => handleSearch(favorites, e.target.value)}
          />
        </span>
        <FontAwesomeIcon
          icon={faSearch}
          className="search__container-input__container-button"
        />
      </div>
      {componentState.searchCities !== [] && (
        <motion.div
          className="search__list"
          animate={componentState.isVisible ? "visible" : "hidden"}
          variants={SearchVariants}
        >
          {componentState.searchCities.map((city) => (
            <div className="search__list-item" key={`${city.lng}${city.lat}`}>
              <motion.div
                className="search__list-item__name"
                whileHover={TextHoverVariant}
                onClick={() => setCurrentLocation(city.lat, city.lng)}
              >
